...
from utils import best_thumb, best_thumb_raw, fmt_song, fmt_thumbs
...
async def search_videos(request: Request, q: str, limit: int = 25):
...
async def trending(request: Request, country: str = "US"):
    ...
    def _do():
        ...
        songs_out = []
        candidates = []
        for key in ("songs", "topSongs", "trendingSongs", "videos", "topVideos"):
        ...
        for r in candidates[:25]:
            ...
            if isinstance(raw_artists, str):
                ...
                artist_str = ", ".join((a.get("name") or a) for a in raw_artists if a)
            else:
                artist_str = ""
            songs_out.append(
                {
        ...
        try:
            ...
            for r in candidates[:30]:
                ...
                if isinstance(raw, str):
                    ...
                    art = ", ".join((a.get("name") or a) for a in raw if a)
                else:
                    art = ""
                songs_out.append(
                    {
